...
boolean[] offState = new boolean[8];

int passedTime;
//--------------------------
int num = 1000;
offVideo[] offVideos = new offVideo[num];
float noiseScale=500, noiseStrength=1;
//--------------------------
boolean activateOffAnimation = false;
...
void setup() {
  ...
  noStroke();
  for (int i=0; i<num; i++) {
    PVector loc = new PVector(random(width*1.9), random(height), 1);
    float angle = random(TWO_PI);
    PVector dir = new PVector(cos(angle), sin(angle));
    offVideos[i]= new offVideo(loc, dir, 2);
  }
  //----------------------------
}
...
//  if ((offCount < 8) &&activateOffAnimation) {
//    background(0);
//  }
   offCount = 0;
  for (int i = 0; i < 8; i++) {
    if (!offState[i]) offCount++;
  }

  if ((offCount == 8)&&(checkVideo)) {
    savedTime = millis();
    println(savedTime);
    checkVideo = false;
  } 
...
void draw() {
  ...
  //-----------------
  if (activateOffAnimation) {
    fill(0, 10);
    noStroke();
    rect(0, 0, width*2, height*2);
    fill(255);  
    for (int i=0; i<offVideos.length; i++) {
      offVideos[i].run();
    }
  }
